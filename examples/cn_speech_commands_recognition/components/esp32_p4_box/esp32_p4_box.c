...
void I2C_scan_test()
{
    ...
    for (i = 0x01; i < 0x70; i++) {
        ...
        i2c_master_stop(cmd);
        if (i2c_master_cmd_begin(BSP_I2C_NUM, cmd, 3000) == ESP_OK) {
            ESP_LOGI(TAG, "0x%02X", i);
        } else {
            //  ESP_LOGW(TAG, "0x%02X", i);
        }
...
esp_err_t bsp_extra_i2c_init(void)
{
...
esp_codec_dev_handle_t bsp_audio_codec_speaker_init(void)
{
    ...
    if (i2s_data_if == NULL) {
        /* Initilize I2C */
        BSP_ERROR_CHECK_RETURN_ERR(bsp_extra_i2c_init());
...
{
    const audio_codec_data_if_t *i2s_data_if = bsp_audio_get_pdm_itf();
    if (i2s_data_if == NULL) {
        /* Configure I2S PDM peripheral */
        BSP_ERROR_CHECK_RETURN_ERR(bsp_microphone_init(NULL));
        i2s_data_if = bsp_audio_get_pdm_itf();
    }
    assert(i2s_data_if);
...
static esp_err_t bsp_enable_dsi_phy_power(void)
{
    ...
    ESP_LOGE(TAG, "MIPI DSI PHY Powered on");
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
    ...
    ESP_GOTO_ON_ERROR(esp_lcd_new_panel_st7701(mipi_dbi_io, &lcd_dev_config, &panel_handle), err, TAG, "New LCD panel ILI9881C failed");
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_i2c((esp_lcd_i2c_bus_handle_t)BSP_I2C_NUM, &io_config, &tp_io_handle), TAG, "");
